...
export function WalletCard({ balanceCents }: { balanceCents: number }) {
  ...
  async function topUp() {
    ...
    try {
      ...
      // Defensive parse: the server may return an empty or non-JSON body
      // (e.g. a 500 from an unhandled error). Never crash the UI.
        ...
        throw new Error(
          data.error ||
            (res.status === 401
              ...
              : "We couldn't start your top-up right now. Please try again in a moment, or contact support if this keeps happening."),
        );
      }
