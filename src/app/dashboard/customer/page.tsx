...
export default async function CustomerDashboard({
      ...
      <div className="container-page py-16">
        <div className="card mx-auto max-w-lg p-10 text-center">
          ...
          <p className="mx-auto mt-2 max-w-sm text-sm leading-relaxed text-slate-600">
            Track your jobs, compare quotes, view professionals you&apos;ve
            worked with and load your rainy-day wallet.
          </p>

          <div className="mt-6 flex flex-col justify-center gap-3 sm:flex-row">
            <Link
              href="/login?next=/dashboard/customer"
          ...
          <p className="mt-4 text-xs text-slate-500">
            If you already signed in but still see this page, refresh the
            browser or open{" "}
            <Link
  ...
  /*
   * Load each dashboard section independently.
   *
   * Previously, Promise.all() meant that ONE failed database query
   * would crash the entire customer dashboard with a server error.
   *
   * Each query now has its own fallback so the dashboard can still
   * render the other sections while the failing query is logged in
   * Vercel Runtime Logs.
   */
  const [myJobs, transactions, workedWith, inbox, wdRows] =
    await Promise.all([
      getJobsForCustomer(customer.id, customer.email).catch((error) => {
        console.error(
          "[Customer Dashboard] Failed to load jobs:",
          error,
        );
        return [];
      }),

      getWalletTransactions(customer.id).catch((error) => {
        console.error(
          "[Customer Dashboard] Failed to load wallet transactions:",
          error,
        );
        return [];
      }),

      getCompletedProfessionals(customer.id, customer.email).catch((error) => {
        console.error(
          "[Customer Dashboard] Failed to load completed professionals:",
          error,
        );
        return [];
      }),

      getInboxMessages(customer.id, customer.email).catch((error) => {
        console.error(
          "[Customer Dashboard] Failed to load inbox:",
          error,
        );
        return [];
      }),

      getWithdrawalsForCustomer(customer.id).catch((error) => {
        console.error(
          "[Customer Dashboard] Failed to load withdrawals:",
          error,
        );
        return [];
      }),
    ...
    <div className="container-page py-8 md:py-12">
      {sp.welcome ? (
        <div className="animate-fade-up mb-6 rounded-2xl border border-emerald-200 bg-emerald-50 px-5 py-4">
          <p className="text-sm font-bold text-good">
            🎉 Welcome to LocalFix SA, {customer.name.split(" ")[0]}!
          </p>

          <p className="mt-1 text-xs text-emerald-800">
            Your profile is ready. Post your first job, or load your wallet so
            you&apos;re covered for a rainy day.
          </p>
        </div>
      ) : null}
      ...
      {sp.topup === "cancelled" ? (
        <div className="animate-fade-up mb-6 rounded-2xl border border-amber-200 bg-amber-50 px-5 py-4">
          <p className="text-sm font-bold text-amber-700">
            Top-up cancelled — no money was taken.
          </p>
        </div>
      ) : null}
      ...
      <section className="mt-6 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat
          ...
          value={formatZAR(customer.walletCents)}
          hint="Ready to spend"
        />
        ...
        <Stat
          label="Active jobs"
          value={active.length}
          hint="Open, quoted or in progress"
        />
      ...
      <div className="mt-8 grid gap-6 lg:grid-cols-[1.6fr_1fr]">
        <div className="space-y-6">
              ...
              {active.map((j) => {
                ...
                return (
                  ...
                  >
                    ...
                    <h3 className="mt-3 text-base font-bold text-navy-800">
                      {j.title}
                    </h3>

                    <p className="text-xs text-slate-500">
                      {j.reference} · {j.suburb || j.city} ·{" "}
                      {j.broadcastCount} pros notified
                    </p>

                    <p className="mt-2 flex flex-wrap items-center gap-3 text-sm">
                      <span className="font-bold text-teal-700">
                        {j.quoteCount} quotes received
                      </span>

                      <span className="text-slate-500">
                        Budget{" "}
                        {j.budgetMin || j.budgetMax
                          ? `${zar(j.budgetMin)} – ${zar(j.budgetMax)}`
                          : "open"}
                      </span>
                    </p>
            ...
            <section>
              ...
              <div className="mt-4 space-y-3">
                {completed.map((j) => (
                  <article
                    key={j.id}
                    className="card card-hover flex flex-wrap items-center gap-3 p-5"
                  >
          ...
          <section className="card p-6">
            ...
            <p className="mt-4 text-xs text-slate-500">
              Pay by wallet, card, instant EFT, PayFast, Ozow or Yoco.
              Deposits and milestone payments supported.
            </p>
          </section>
            ...
            {workedWith.length === 0 ? (
              <div className="card mt-4 p-6 text-center">
                ...
                <p className="mt-1 text-xs text-slate-600">
                  Once you complete a job through LocalFix SA, your
                  professional will appear here automatically.
                </p>
              </div>
            ) : (
                  ...
                  >
                    <div className="flex items-start gap-3">
                      {entry.provider.logoUrl ? (
                        ...
                        <span
                          ...
                          style={{
                            backgroundColor: entry.provider.accent,
                          }}
                          aria-hidden
                        >
                    ...
                    <div className="mt-3 flex flex-wrap items-center gap-2 text-xs text-slate-600">
                      <span className="font-semibold text-navy-800">
                        {entry.completedJobCount} completed job
                        {entry.completedJobCount === 1 ? "" : "s"}
                      </span>
                      ...
                      {entry.lastCompletedAt ? (
                        <>
                          ...
                          <span>
                            Last:{" "}
                            {entry.lastCompletedAt.toLocaleDateString(
                              "en-ZA",
                              {
                                day: "numeric",
                                month: "short",
        ...
        <aside className="space-y-5">
          <div className="card p-5">
            ...
            {inbox.length === 0 ? (
              <p className="mt-3 text-sm text-slate-600">
                Quotes from verified professionals will land here
                automatically.
              </p>
            ) : (
                  ...
                  >
                    <span
                      className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${
                        m.read ? "bg-slate-200" : "bg-teal-500"
                      }`}
                      aria-hidden
                    />
            ...
            <dl className="mt-3 space-y-2 text-sm">
              {[
                ...
                >
                  ...
                  <dd className="truncate text-right font-semibold text-navy-800">
                    {v}
                  </dd>
                </div>
              ))}
          ...
          <div className="card p-5">
            ...
            <button className="btn btn-ghost mt-3 w-full !py-2.5 text-sm">
              + Add property
            </button>
          </div>
          ...
          <div className="card p-5">
            ...
            <p className="mt-2 text-sm text-slate-600">
              Share your code and both you and your friend get R150
              credited to your wallets.
            </p>
